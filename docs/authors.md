# Authors and Citation

## Authors

- **[Gabriel Hoffman](http://gabrielhoffman.github.io)**. Author,
  maintainer. [](https://orcid.org/0000-0002-0957-0224)

## Citation

Source:
[`DESCRIPTION`](https://github.com/GabrielHoffman/fastglmm/blob/HEAD/DESCRIPTION)

Hoffman G (2026). *fastglmm: Efficiently Fit Generalized Linear Mixed
Model with a Single Random Effect on Massive Datasets*. R package
version 0.4.13, <https://gabrielhoffman.github.io/fastglmm/>.

@Manual{, title = {fastglmm: Efficiently Fit Generalized Linear Mixed
Model with a Single Random Effect on Massive Datasets}, author =
{Gabriel Hoffman}, year = {2026}, note = {R package version 0.4.13}, url
= {https://gabrielhoffman.github.io/fastglmm/}, }
